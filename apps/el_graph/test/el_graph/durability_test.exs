defmodule ElGraph.DurabilityTest do
  ...
  describe ":exit" do
    test "최종 체크포인트만 영속한다 (중간 step 없음)", %{cp: cp, config: config} do
      assert {:ok, _} =
               ElGraph.invoke(linear_graph(), %{}, checkpointer: cp, thread_id: "t", durability: :exit)
    ...
    test "동적 인터럽트는 그래도 영속돼 재개 가능하다", %{cp: cp, config: config} do
      assert {:interrupted, _} =
               ElGraph.invoke(ask_graph(), %{}, checkpointer: cp, thread_id: "t", durability: :exit)
  ...
  describe ":async" do
    test "반환 전 flush되어 모든 체크포인트가 영속된다", %{cp: cp, config: config} do
      assert {:ok, _} =
               ElGraph.invoke(linear_graph(), %{}, checkpointer: cp, thread_id: "t", durability: :async)
    ...
    test "인터럽트가 내구적으로 영속돼 재개 가능하다", %{cp: cp, config: config} do
      assert {:interrupted, _} =
               ElGraph.invoke(ask_graph(), %{}, checkpointer: cp, thread_id: "t", durability: :async)

defmodule ElGraph.LLM.OpenAIStreamTest do
  ...
  defp decode_all(chunks) do
    {deltas, _state} =
      Enum.reduce(chunks, {[], OpenAI.init_stream_state()}, fn chunk, {acc, st} ->
        {ds, st} = OpenAI.decode_deltas(chunk, st)
        {acc ++ ds, st}
      end)
  ...
  describe "decode_deltas/2 — per-chunk delta events (stateful)" do
    test "yields a token event for a content delta" do
      assert {[{:token, "Hello"}], _} =
               OpenAI.decode_deltas(text_chunk("Hello"), OpenAI.init_stream_state())
    end
    ...
    test "emits start/delta/end across an incrementally streamed tool call" do
      chunks = [
        %{
          "choices" => [
            %{
      ...
      assert [
               {:tool_call_start, "call_1", "web_search"},
               {:tool_call_delta, "call_1", "{\"q"},
               {:tool_call_delta, "call_1", "\":\"x\"}"},
    ...
    test "does not re-emit tool_call_start for the same index" do
      ...
      assert [{:tool_call_start, "call_1", "f"}, {:tool_call_delta, "call_1", "a"}] =
               decode_all(chunks)
    end
  end

  describe "decode_usage/1" do
    test "extracts usage from a usage chunk" do
    ...
    test "nil when the chunk has no usage" do
      assert nil == OpenAI.decode_usage(text_chunk("hi"))
    end
  end
